...
async def chat_with_oracle(request: ChatMessage, db: AsyncSession = Depends(get_db)):
    ...
    if not products:
        ...
        for p in products:
            # 🔥 Убрали .0 с помощью int() и добавили двойной перенос \n\n
            catalog_text += f"🔹 **{p.title}** | ${int(p.price)}\n{p.description}\n\n"
    ...
    Пиши в стиле markdown, чтобы на фронтенде было красиво. Не делай много переносов строки максимум 1-2. Не придумывай ничего сверх того, 
